...
date: "Revised: Ocotober 24th, 2019"
output:
...
It can also help to mitigate the technical or batch effects on the analysis, a feature which we'll take advantage of here. 

Most of these samples are 150 bp, paired-end sequencing (as shown in the 'lib' column of the sample sheet).
A handful are 50 bp, single-end sequencing, which introduces a significant confounding factor to the analysis.
In our model design, we'll specify that we want to ignore effects due to this technical difference (by setting `block = "lib"`).
The model isn't magic, so we may very well still see evidence of the library-prep differences regardless, as they are quite strong.
